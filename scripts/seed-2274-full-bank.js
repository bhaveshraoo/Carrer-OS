...
console.log("📖 Loading 2,274 questions from JSON bank...");
...
async function seedDatabase() {
  console.log(`🧹 Clearing existing questions from Supabase dsa_questions table...`);
  ...
  console.log(`🚀 Preparing 2,274 questions for insertion...`);
  ...
  const chunkSize = 100;
  console.log(`📦 Seeding ${formattedQuestions.length} questions in chunks of ${chunkSize}...`);
  ...
  for (let i = 0; i < formattedQuestions.length; i += chunkSize) {
    ...
    if (insErr) {
      ...
      console.log(`✅ Chunk ${i + 1}..${i + chunk.length} inserted successfully (${successCount}/${formattedQuestions.length})`);
    }
  }

  console.log(`🎉 SUCCESS! ${successCount} / ${formattedQuestions.length} questions seeded into Supabase!`);
}
